...
class Explosion:
    ...
    def activate(self,x,y):
        ...
        self.r = 0
        self._active = True
    #checks whether the explosion is active
    def is_active(self):
        return self._active
    #deletes the explosion from the canvas
    def deactivate(self):
        for dot in self.dots:
            self.canvas.delete(dot.dot)
        self._active = False
    #determines the next place of each pixel in the explosion, using quadrants
    def next(self):
        if self._active:
            self.r += 1
    ...
    #adds the explosion to the canvas
    @staticmethod
    def add_explosion(canvas, booms, center_x, center_y, max_size=80, color = "rainbow"):
        ...
        booms.append(boom)
# A different kind of explosion with physics instead
class Explosion_gravity(Explosion):
    ...
    #determines where the next pixel in the more realistic firework will go
    def next(self):
        if self._active:
            self.r +=1
            g = 0.06

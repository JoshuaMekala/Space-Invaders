...
class Alien:
    ...
    def __init__(self, canvas, increment=4, color="yellow", w=50, h=50, value=1):
        ...
        self.h = h
        self.value = value
    #set's up where the x and y coordinates of the alien will be 
    ...
    #returns the value of whether the alien is active or not
    def is_active(self):
        return self._active

    # deletes the alien from the canvas
    def deactivate(self):
        self.canvas.delete(self.alien)
        self._active = False
    # Moves the alien down by pixel increments 
    def next(self):
        if self._active == True:
            ...
            if self.y > self.canvas.winfo_height():
                self.deactivate()
    # determines whether the alien has been shot or not
    def is_shot(self, x, y):
        if x > self.x1 and x < self.x2 and y > self.y1 and y < self.y2:
            return True
        else:
            return False
    # add's random aliens to the canvas
    @staticmethod
...
###############################################################
################################################################
#given shoot displays prompt depending on whether the alien is hit or not
def shoot(alien,x,y):
